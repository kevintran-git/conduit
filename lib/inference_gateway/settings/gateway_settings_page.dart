...
class _GatewaySettingsPageState extends ConsumerState<GatewaySettingsPage> {
  ...
  _Gate _toolsGate(GatewayConfig cfg) {
    final base = _credentialsGate(cfg);
    if (base.inactive) return base;
    if (!cfg.chatEnabled && !cfg.voiceEnabled && !cfg.realtimeEnabled) {
      return _reasonGate(
        'Inactive — enable Chat completions or a call mode above.',
      );
    }
    return _active;
  }
    ...
    return SettingsPageScaffold(
      ...
      children: [
        ...
        _buildToggleTile(
          ...
          subtitle: 'Chat requests go to your gateway.',
          value: cfg.chatEnabled,
          onChanged: notifier.setChatEnabled,
          gate: _credentialsGate(cfg),
        ),
        const SizedBox(height: Spacing.sm),
        _buildToggleTile(
          context: context,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _StatusLine(
              label: 'Chat routing',
              value: chatActive ? 'Active' : 'Off — using Open WebUI',
              ok: chatActive,
            ),
          ],
        ),
