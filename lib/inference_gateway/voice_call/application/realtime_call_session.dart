...
class RealtimeCallSession extends Notifier<CallSessionState>
  ...
  ik.LiveClient? _client;
  CallBackgroundLease? _backgroundLease;
  StreamSubscription<Uint8List>? _micSub;
  StreamSubscription<ik.LiveEvent>? _eventsSub;
  ...
  void _ensureTurnMessages() {
    ...
    final userMessage = ChatMessage(
      ...
      metadata: {
        'parentId': parentId,
        'childrenIds': <String>[assistantId],
      },
    );
  ...
  Future<void> _connect({String? resumeHandle}) async {
    ...
    final tools = await ref
        .read(gatewayToolRegistryProvider)
        .buildTools(
          includeStatsTool: cfg.statsToolEnabled,
    ...
    final client = ik.LiveClient(dio: ref.read(gatewayClientProvider).dio);
    _client = client;
    try {
      await client.start(
        model: cfg.callModel,
  ...
  void _onLiveEvent(ik.LiveEvent event) {
    if (!_alive) return;
    switch (event) {
      case ik.LiveAudioChunk(:final bytes):
        _playChunk(bytes);
      case ik.LiveInputTranscript(:final text):
      ...
      case ik.LiveOutputTranscript(:final text):
        ...
        _ensureTurnMessages();
        _syncAssistantContent();
      case ik.LiveInterrupted():
        _handleInterrupted();
      case ik.LiveTurnComplete():
        _endTurnAudio();
      case ik.LiveToolCall(:final calls):
        unawaited(_handleToolCall(calls));
      case ik.LiveError(:final message):
        DebugLogger.warning(
          'live-error',
          scope: 'call/live',
  ...
  Future<void> _handleToolCall(List<ik.LiveFunctionCall> calls) async {
    if (!_alive) return;
