...
void main() {
  ...
  test('falls back to the default base URL when none is configured', () async {
    ...
    check(env.adapter.lastOptions!.baseUrl)
        .equals(GatewayConfig.defaultBaseUrl);
  });

  test(
    'reads the API key live, so updates apply without recreating the client',
    () async {
      final env = _gatewayClient(
        GatewayConfig.defaults().copyWith(
          baseUrl: 'https://gateway.test',
      ...
      env.container.read(gatewayConfigProvider.notifier).state = env.container
          .read(gatewayConfigProvider)
          .copyWith(apiKey: 'second');

      await env.client.dio.get<void>('/v1/models');
      check(env.adapter.lastOptions!.headers['Authorization'])
          .equals('Bearer second');
    },
  );
}
