...
void main() {
  ...
  test('starting a call hides non-direct tool ids without discarding them', () {
    ...
    check(container.read(selectedToolIdsProvider))
        .deepEquals(['direct_server:abc']);

    endCall(container);
    check(container.read(selectedToolIdsProvider))
        .deepEquals(['builtin_search', 'direct_server:abc']);
  });
  ...
  test('starting a call clears the terminal selection and restores it once it ends', () {
    final container = makeContainer();
