...
import 'package:conduit/inference_gateway/audio/gateway_live_client.dart';
...
void main() {
  test('routes each call to the matching tool by name', () async {
    ...
    final results = await executeLiveToolCalls([
      LiveFunctionCall(id: 'call-1', name: 'echo', args: {'text': 'hi'}),
    ], tools);
  ...
  test('an unknown tool name returns an error response instead of throwing', () async {
    final results = await executeLiveToolCalls([
      LiveFunctionCall(id: 'call-1', name: 'missing', args: const {}),
    ], const {});

    check(results.single.response['error']).equals('Unknown tool: missing');
  });

  test('a handler that throws is caught and wrapped as an error response', () async {
    final tools = {
      'boom': _tool('boom', (args) async => throw StateError('kaboom')),
    };

    final results = await executeLiveToolCalls([
      LiveFunctionCall(id: 'call-1', name: 'boom', args: const {}),
    ], tools);
  ...
  test('multiple calls run and preserve per-call id/name pairing', () async {
    ...
    final results = await executeLiveToolCalls([
      LiveFunctionCall(id: '1', name: 'a', args: const {}),
      LiveFunctionCall(id: '2', name: 'b', args: const {}),
    ], tools);
